...
| D. Event model | ✅ continuous + jump + **discrete/branching** built | EXP-035 (continuous heteroskedastic diffusion, beats persistence 24% on CRPS) + **EXP-077** (`swm/simulation/event_model.py`: calibrated *continuous-jump* variance placement — 82% interval coverage vs persistence's 3%) + **EXP-083: the *discrete categorical* term — `swm/transition/future_events.py`: `FutureEvent`s whose categorical outcomes JUMP the belief or RESOLVE the question, a `SurpriseHazard`, an `EventCalendar`, and an `EventImpactJudge` (LLM). Categorical forks (hold/cut) that EXP-077's Gaussian jumps cannot express; the terms compose as one jump-diffusion.** Open: real calendar + LLM-authored impacts scored no-cheat. |
| E. Rollout engine | ✅ distributional MC + **branching** built | one-step transition (EXP-030), unified form (EXP-032), heteroskedastic rollout (EXP-035), bottom-up aggregation (EXP-034), and **EXP-083: `swm/simulation/branching_rollout.py` — Monte-Carlo over sampled event trajectories with a pluggable continuous step (the EXP-077/transition-operator dynamics drop in) + pivotal-branch decomposition.** Open: coupling the *population* rollout to the event rollout. |
| F. Outcome distribution | ✅ calibrated + **pivotal branches** built | EXP-035: CRPS-scored horizon-calibrated bands. **EXP-083: pivotal-branch conditionals — "25% if hot, 80% if cool, P(each)" — plus a reducible/irreducible split that replaces abstention (model past the horizon as a branching distribution, never a false point).** Open: joint distributions across coupled questions. |
| G. Decision/action | ✅ built | the general action layer (`swm/decision/`, EXP-069: `argmax_a E[U|do(a)]`, best-arm racing, +22pt on real CMV) and **EXP-083: best-action on FORWARD questions — `simulate_forward(actions=…)` scores a `do()` that shifts an event's odds by rolling the world forward under it (persistence cannot evaluate this).** |
...
bottom-up aggregation ✅ · calibrated multi-step distributional rollout ✅ (EXP-035) · **future-event model +
branching-realities rollout ✅ (EXP-083)** · **pivotal-branch decomposition ✅ (EXP-083)** · **decision/action
layer ✅ (EXP-069) incl. best-action on forward questions ✅ (EXP-083)**. The forward core now models past the
near-horizon as a *branching distribution* (never a false point, never abstention) and chooses the best
action over it. **Remaining: (i) a real future-event calendar with LLM-authored impacts scored no-cheat
(the event→impact authoring is the LLM's job + its own calibration); (ii) coupling the *population* rollout
to the event rollout; (iii) the calibrated *continuous* transition operator that drops into the branching
engine's `continuous_step` seam (in progress separately) — together the two terms of one jump-diffusion.**

**EXP-083 — branching-realities rollout: modeling FORWARD through future events, never faking a point (the hard core, built).** EXP-033's verdict was that long-horizon is a *future-event-forecasting* problem, not a better-dynamics problem — and that piece (Stage D discrete term + Stage E branching) was the one genuinely-missing core, NOT another front door (`simulate(question)` already exists ~6×). Distinct from EXP-077's `event_model.py` (which places *continuous-Gaussian-jump* variance for interval coverage): this is the *discrete categorical* term — the branching + pivotal decomposition + conditional forecast + forward best-action that a summed-Gaussian model structurally cannot produce; the two compose via the pluggable `continuous_step` seam. Built: `swm/transition/future_events.py` (dated `FutureEvent`s with categorical outcome distributions that JUMP the belief or RESOLVE the question; a `SurpriseHazard`; an `EventCalendar`; an `EventImpactJudge` that authors it from a retrieved timeline via the EXP-030 LLM channel) + `swm/simulation/branching_rollout.py` (Monte-Carlo over sampled event trajectories with a **pluggable continuous step** — the calibrated transition operator drops in there, so this is the *discrete* term of a jump-diffusion, composing with not colliding with that work — plus `pivotal_branches` and a `forward_forecast` that returns distribution + forks + a reducible/irreducible split that **replaces abstention**). Wired into `WorldModel.simulate(question, events=…)` → `simulate_forward`, with best-action on forward questions. No-cheat generative validation (1,500 instances, model sees only b0 + event structure): (1) BLIND MARGINAL ties persistence (Brier skill −0.001 — never fakes a point, EXP-033 respected); (2) CONDITIONAL, once the pivot resolves, WINS **+0.245 Brier / +0.188 log-loss skill** — the payoff of modeling the event is the fork, not the marginal; (3) pivotal recovery 100%, conditional rates recovered (0.239/0.751 ≈ true b0∓0.25); (4) best-action scores a `do()` that shifts an event's odds (+0.159 lift) that persistence cannot evaluate; (5) honest negative — no events → collapses to persistence (0.000). This is "model past the horizon as a branching distribution, never a false point" realized. `swm/transition/future_events.py`, `swm/simulation/branching_rollout.py`; 16 new tests; full suite 365. Named next: a real FOMC/election calendar with LLM-authored impacts scored no-cheat.
...
**EXP-090 — The ask-the-user flow, wired into the live front door.** The dossier + grounded-inference stack is now reachable through the one call a user makes. `swm/api/person_intake.py` (`PersonIntake`) runs a preflight at the top of `WorldModel.simulate`: (1) identify whether the question turns on a SPECIFIC individual's disposition (vs an institution/market/population) and which of their personal variables decide it; (2) assemble a dossier (user `context` first, then history, then web footprint); (3) route — THIN evidence → return `mode:"needs_user_context"` with the exact questions to ask (short-circuit; never fabricate a person's mood/openness), SUFFICIENT → fold the dossier into context, infer the person-variables through the three pillars, run the calibrated sim. `general_world_model(person=True)` wires it by default (self-disables with no LLM key; existing behavior untouched — `person_intake=None` skips the preflight). Live (DeepSeek): "Will Marcus grab coffee if I text him?" → ASK (identifies Marcus + relationship/personality/history/topic questions); "Will US inflation be <3%?" → proceed (person=None); "Will Marcus grab coffee?" + "college roommate, close friend" → proceed, dossier strength 0.65, 3 person-variables inferred. So a question about a specific person automatically triggers the ask-the-user experience. ANSWERING AUTO-RUNS: `simulate(question, answers=<answers>)` folds the answers into context so the SAME call proceeds through dossier → grounded person-variables → calibrated run (no separate step; the ask result carries a `resume_hint`; answers accept str/list/{q:a} dict) — verified live (ask 4 questions → answer → compiler runs on the user's context, person=Marcus). tests/test_person_intake.py (7). Report: experiments/exp090_person_intake_frontdoor.md.

**EXP-089 — Grounded inference is now the DEFAULT for every variable grounding, + the private-individual path.** Wired EXP-086's three pillars into the live grounding stack (which already had PR #61's web-search auto-grounder = Pillar 1 evidence + Pillar 3 CI-calibration). `swm/api/anchored_extractor.py` (`AnchoredExtractor`) adds Pillar 2 (reference-class outside-view anchor) + Pillar 3 (ensemble + empirical-Bayes shrink-to-base-rate via `grounded_inference.shrink`); `build_retrieval_grounder(anchor=True)` makes it the default, so every `general_world_model()` grounding now runs the full pillar stack + web + structured feeds. SELF-REGULATING (safe as default): strong/precise web evidence (small sd) is left untouched — EXP-087/088 live macro/market grounding unchanged — while weak grounding degrades to the honest outside view instead of a confident guess (full suite 458 passing). The private-individual path (the case the product faces — someone you can't look up): `swm/variables/dossier.py` assembles a dossier from USER-SUPPLIED CONTEXT (relationship, how you met, your read on them — highest signal, no dataset has it) + message history + web footprint, with an honest `needs_user_context`/`context_questions` gate that ASKS the user when the footprint is thin rather than guessing, then infers each variable through the pillar stack. EXP-089 live demo: thin context fires the ask-gate; user context assembles a dossier that tightens 3/3 estimates and moves 3/3 off the reference-class base rate (relationship +0.10 stranger base rate → +0.20 on the evidence) — the 23%→87% mechanism now driven by what the user knows. Tests: test_anchored_extractor (4), test_dossier (4). Reports: experiments/exp089_grounded_inference_default.md. (EXP-086 remains the quantitative proof on measurable ground truth; private individuals lack a clean latent-trait yardstick, so the mechanism is proven there and applied here.)

**EXP-086 — How close can an INFERRED variable get to a MEASURED one? (the three-pillar grounded-inference engine, scored).** The project's core question: we won't always have 7,000 votes to MEASURE a variable, so can we INFER it well enough? Built `swm/variables/grounded_inference.py` (a Bayesian estimator with a MEASURED prior + CALIBRATED likelihood: `ensemble_infer` = Pillar 3 self-consistency spread, `reference_prior` = Pillar 2 measured base rate, `fit_calibration`/`apply_calibration` = Pillar 3 LLM→truth map learned where truth exists, `shrink`/`grounded_estimate` = Pillar 3 empirical-Bayes pull to base rate unless evidence earns the deviation; 7 unit tests). Tested on the EXP-085 yardstick: HIDE every senator's votes, INFER ideology from stranger-info (name/state/party/era + LLM knowledge), plug into the identical committee-vote world model. Ladder (floor=party mean 0.856 → ceiling=measured 0.937): **llm_named inference reaches 0.926 = 87% of the gap closed, ideology corr with truth 0.96** — an inferred variable is MOST of a measurement, with zero ground-truth on the target. Pillar ranking (honest): **Pillar 1 EVIDENCE is the giant lever** (name given vs withheld = 87% vs 23% of gap — the LLM's retrieved footprint is nearly everything); **Pillar 2 reference-class** is the floor when the individual is unknown (23% > 0); **Pillar 3 anchor/calibrate ~neutral for confident public-figure inferences** (it correctly declined to shrink tight-spread estimates — a safety net for NOISY inferences, not these). Honest boundary: senators are public figures so this is the well-documented-entity ceiling; a private individual falls back to the ~23% reference-class regime UNLESS you supply a dossier — which is exactly why context-revival (Pillar 1) is the product. One-liner: demote the LLM from oracle to (retriever + reference-class classifier); measured on real data that turns a guess into 87% of a measurement, and the missing 13% is bought with more evidence, not a better model. Report: experiments/exp086_inference_quality.md.

**EXP-085 — The committee-vote world model: measured-state agents FINALLY beat the compact shortcut (the corrected Maximal World thesis).** After EXP-083/084 (agents with GUESSED state lose), the critique: fidelity needs MEASURED state (grounding, EXP-082's +58pt lever) on a high-ceiling structure-dominated question. Built exactly that on VoteView Senate roll-calls (7,233 divided votes, congresses 106-118, ~694k member-votes, leakage-free). Each member is an agent whose ideology is GROUNDED from their PRIOR congress (measured before the vote, not fit from it); 80/20 member split per bill, predict the held-out 20%. Results: base-rate 0.648 acc, party shortcut (the compact "everyone votes their party line", ~90% in a polarized chamber) 0.902, **grounded AGENT world 0.916 (+0.014 overall, +0.016 on CONTESTED votes, Brier 0.064 vs 0.072)** — a ~15% relative error cut over a very strong baseline, statistically overwhelming at 130k held-out votes, winning MORE where party-line is weakest. Vivid mechanism: on 13,683 DEFECTOR votes (member voted against their party majority) the party shortcut is 0% by construction while the grounded agent catches 22% — measured individual ideology predicts WHICH moderates cross the aisle, signal the group summary cannot represent. **The variable that flips agents-lose→agents-win is GUESSED vs MEASURED state**, not agents-vs-no-agents. Decision rule proven from both sides: simulate individuals as agents where state is measurable AND the outcome is structure-dominated (committees/courts/roll-calls); use the compact model for aggregate-only or irreducibly-noisy questions (what the EXP-078 router does). Report: experiments/exp085_committee_vote_world.md; data via experiments/build_congress_votes.py.

**EXP-083/084 — The Maximal World: does modeling every person as an agent beat the compact shortcut? (answering "we're not modeling the whole world yet").** Built the maximal thing the user asked for — model every relevant person as an agent with full context on a shared plane, roll forward — and scored it no-cheat head-to-head against the calibrated compact model + baselines in the two named regimes. **Part A (cascade, baby-name fashion tipping, 481 names, H=10yr, leakage-free):** heterogeneous-threshold + fatigue agents (Granovetter) vs the mean-field ODE shortcut (EXP-072). Agent world BEATS the dumb baselines at turning points (+31% vs persistence) — the coupled dynamics are real — but LOSES to the mean-field (0.182 vs 0.152 MAE, −20%); modeling individuals recovered the SAME tipping signal the 2-param ODE already had, at higher complexity. The law: for an AGGREGATE observable the individual threshold distribution is UNIDENTIFIABLE (infinitely many agent populations give the same sum) → it collapses to the mean-field. (First agent build was a strawman that blew up / drifted on stable names; fixed to a physically-sane relaxation-with-fatigue model anchored to the observed share before drawing the conclusion.) **Part B (best-launch-messaging, CMV, which reply flips THIS person, 200 held-out OPs — the individual-target regime where agents SHOULD pay):** model the target as an agent (persona then simulate) = 0.44 precision@1, BELOW random (0.512) and −0.20 vs the shortcut (LLM one-shot, each reply scored INDEPENDENTLY, 0.640). Controlled decomposition: the persona step added nothing (−0.055 vs a no-persona joint control 0.495); the real damage is the JOINT-COMPARATIVE framing (0.640→0.495) — asking the LLM to rank candidates together is far worse than scoring each alone (actionable: score each candidate message independently, never head-to-head). **Net (both regimes): the maximal world does NOT beat the calibrated shortcut** — world-completeness is not the missing lever; identifiability + irreducible noise set the ceiling. Model individuals as agents only where the target is an individual/small-group, identifiable, and interacting; route aggregate questions to the compact model (what the EXP-078 regime router already does). Turns "model everything" from a belief into a decision rule. Report: experiments/exp083_084_maximal_world.md.
