...
def main():
    ...
    failures = [
        ...
        {"failure_id": "no_verified_templates_for_agency_queue_election_moderation",
         "family": "administrative_agency/queue_capacity_service/electoral_administration/moderation_appeals",
         "failure_type": "evidence_gap",
         "detail": "The agency/queue/election/moderation FAMILIES are executable, but no template-specific "
                   "official evidence was core-verified this run, so no evidence-backed TEMPLATE exists for "
                   "them — they select at Tier 3 (structural) with rule uncertainty. Honest gap, not faked.",
         "artifact": SUMM},
        {"failure_id": "scotus_cert_count_not_fraction", "template": "scotus_certiorari",
         ...
         "detail": "The Rule of Four is a COUNT (4 of 9), an informal custom; it is stored as evidence but "
                   ...
                   "count. SCOTUS cert stays `executable` (informal), not historically replayed.",
         "artifact": SUMM},
    ]
