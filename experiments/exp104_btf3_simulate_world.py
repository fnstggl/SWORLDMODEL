...
def run() -> dict:
    ...
    for qid in QIDS:
        ...
        # operator census: which operators actually wrote outcome state
        census = (d.get("provenance") or {}).get("operator_delta_census")
        traces.append({"qid": qid, "question": q["question"], "n_llm_calls": len(calls),
                       ...
                       "mechanism_tiers": d.get("mechanism_tiers"),
        ...
        results.append({"qid": qid, "question": q["question"][:110], "status": d.get("simulation_status"),
        ...
        print(f"  {qid[:8]}  status={d.get('simulation_status')}  p={p}  calls={len(calls)}  "
              f"ops={sorted((census or {}).keys()) if census else []}")
