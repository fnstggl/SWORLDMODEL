...
def _ask(llm, prompt):
    ...
    try:
        # Probe evidence must be independently auditable. These artifacts are
        # scorer-side and sealed from the forecaster, so retain the exact full
        # prompt rather than a lossy preview.
        return {"prompt": prompt, "output": parse_json(llm(prompt)) or {"parse_failed": True}}
    except Exception as e:  # noqa: BLE001
        return {"prompt": prompt, "output": {"error": str(e)[:100]}}
...
def run_probes_v2(llm, *, real_question: str, blinded_question: str, mapping: dict,
    ...
    if evidence_text:
        ...
        if flipped == evidence_text:
            out["counterfactual_evidence"] = {"skipped": "no directional cue could be flipped"}
        else:
...
def classify_row_v2(probes: dict, *, arm: str, name_only_correct: bool | None) -> str:
    ...
    perm = probes.get("identity_permutation") or {}
    if "base" in perm and "permuted" in perm:
        base_p = ((perm["base"].get("output") or {}).get("p_yes"))
        perm_p = ((perm["permuted"].get("output") or {}).get("p_yes"))
        if isinstance(base_p, (int, float)) and isinstance(perm_p, (int, float)) and \
                abs(base_p - perm_p) >= 0.15:
            return "contamination_susceptible"
    ...
    temporally_positive = (temporal.get("already_resolved") is True or
                           ...
                           ("definitively failed" in temporal_answer and
                            "not definitively failed" not in temporal_answer))
    if float(temporal.get("confidence", 0) or 0) >= 0.7 and temporally_positive:
        return "contamination_susceptible"
    parse_issues = _has_parse_issue(probes)
    if parse_issues:
...
def _has_parse_issue(value) -> bool:
    if isinstance(value, dict):
        if value.get("parse_failed") or value.get("error"):
            return True
        return any(_has_parse_issue(item) for item in value.values())
    if isinstance(value, list):
        return any(_has_parse_issue(item) for item in value)
    return False
