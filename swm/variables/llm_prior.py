...
# A GENERAL, per-question prior store. The hardcoded AXIS/ITEM_POLE above are the *cached* GSS example
# (reproducibility); for an arbitrary question the prior is GENERATED by an LLM and registered here — the
# axis map is not scenario-specific machinery, only this example's contents are.
_LEARNED_PRIOR: dict = {}       # item -> {(attr, level): signed_value}


def register_prior(item: str, table: dict) -> None:
    """Register an LLM-generated prior for a question: {(attr, level): signed push toward YES in [-1,1]}."""
    _LEARNED_PRIOR[item] = dict(table)


def prior_from_llm(item: str, question: str, attrs: list, levels: dict, judge_fn) -> dict:
    """Generate the prior for ANY question via an LLM (wired like semantic_stance) and register it.

    `judge_fn(prompt) -> {"(attr,level)": signed_value, ...}` (or raw JSON). `levels` maps each attribute
    to its possible values. The LLM supplies, from world knowledge, how each attribute level pushes the
    question toward YES — the general replacement for the hardcoded GSS AXIS table.
    """
    import json as _json
    lines = [f"QUESTION (resolves YES/NO): {question}", "For each attribute LEVEL below, give a signed "
             "number in [-1,1]: how it pushes a person toward answering YES (world knowledge, coarse).",
             "Attributes and levels:"]
    for a in attrs:
        lines.append(f"  {a}: {', '.join(map(str, levels.get(a, [])))}")
    lines.append('Return ONLY compact JSON mapping "attr::level" -> number.')
    raw = judge_fn("\n".join(lines))
    obj = raw if isinstance(raw, dict) else _json.loads(raw[raw.find("{"):raw.rfind("}") + 1])
    table = {}
    for k, v in obj.items():
        if "::" in str(k):
            a, l = str(k).split("::", 1)
            try:
                table[(a, l)] = max(-1.0, min(1.0, float(v)))
            except (TypeError, ValueError):
                pass
    register_prior(item, table)
    return table


def prior_value(item: str, attr: str, level: str) -> float:
    """The prior: signed push of (attr=level) toward this item's YES pole, in ~[-1,1]. Uses an
    LLM-registered per-question prior when available (general), else the cached GSS AXIS example."""
    learned = _LEARNED_PRIOR.get(item)
    if learned is not None:
        return learned.get((attr, level), 0.0)
